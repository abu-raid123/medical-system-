...
function InquiryContent() {
  ...
  const initialLeaveId = searchParams.get("leaveId") ?? "";

  const [leaveId, setLeaveId] = useState(initialLeaveId);
  ...
  async function handleSearch(searchId?: string) {
    ...
    const res = await fetch(`/api/reports?leaveId=${encodeURIComponent(id)}`);
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          <div className="flex gap-3">
            ...
            <button
              ...
              disabled={loading}
              className="bg-[#2B3D77] hover:bg-[#1e2d5a] text-white font-bold py-3 px-8 rounded-lg transition disabled:opacity-50"
            >
              {loading ? "جاري البحث..." : "بحث / Search"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-6">
        {loading && (
          <div className="text-center py-10">
            <div className="animate-spin h-10 w-10 border-4 border-[#2B3D77] border-t-transparent rounded-full mx-auto mb-4" />
              ...
              <table className="w-full">
                <tbody>
                  <DetailRow
                    labelEn="Name"
                    labelAr="الاسم"
                    valueEn={report.nameEn}
                    valueAr={report.nameAr}
                  />
                  <DetailRow
                    labelEn="National ID / Iqama"
                    labelAr="رقم الهوية / الإقامة"
                    valueEn={report.nationalId}
                    valueAr={report.nationalId}
                  />
                  <DetailRow
                    labelEn="Nationality"
                    labelAr="الجنسية"
                    valueEn={report.nationalityEn}
                    valueAr={report.nationalityAr}
                  />
                  <DetailRow
                    labelEn="Employer"
                    labelAr="جهة العمل"
                    valueEn={report.employerEn}
                    valueAr={report.employerAr}
                  />
                  <DetailRow
                    labelEn="Admission Date"
                    labelAr="تاريخ الدخول"
                    valueEn={report.admissionDate}
                    valueAr={report.admissionDateHijri}
                  />
                  <DetailRow
                    labelEn="Discharge Date"
                    labelAr="تاريخ الخروج"
                    valueEn={report.dischargeDate}
                    valueAr={report.dischargeDateHijri}
                  />
                  <DetailRow
                    labelEn="Duration"
                    labelAr="مدة الإجازة"
                    valueEn={`${report.durationDays} day(s)`}
                    valueAr={`${report.durationDays} يوم`}
                  />
                  <DetailRow
                    labelEn="Practitioner"
                    labelAr="الممارس"
                    valueEn={report.practitionerNameEn}
                    valueAr={report.practitionerNameAr}
                  />
                  <DetailRow
                    labelEn="Position"
                    labelAr="المسمى الوظيفي"
                    valueEn={report.positionEn}
                    valueAr={report.positionAr}
                  />
                  <DetailRow
                    labelEn="Hospital"
                    labelAr="المستشفى"
                    valueEn={report.hospitalNameEn}
                    valueAr={report.hospitalNameAr}
                  />
                </tbody>
              </table>
    ...
    <Suspense
      fallback={
        <div className="min-h-screen flex items-center justify-center">
          <div className="animate-spin h-10 w-10 border-4 border-[#2B3D77] border-t-transparent rounded-full" />
        </div>
      }
    >
      <InquiryContent />
